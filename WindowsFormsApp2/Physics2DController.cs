...
namespace WindowsFormsApp2
{
    ...
    {   // находится ли мячик вне границ массива + столкновение с чем-либо
        ...
        public bool IsCollade(Player player, MapController map, Label ScoreLabel, Label ScoreNubmerLabel)
        {
            ...
            if (map.BallX + map.dirX > MapController.mapWidth - 1 || map.BallX + map.dirX < 0)
            {
                map.dirX *= -1; // мяч полетит в другю сторону
                isColliding = true;
            }
            // столкновение с границей по у
            if (map.BallY + map.dirY > MapController.mapHeight - 1 || map.BallY + map.dirY < 0)
            {
            ...
            ScoreNubmerLabel.Text = player.score.ToString();
            return isColliding;
